...
dir_path <- c("./modeling_results/mono/")
...
scoringFunction <-
  function(eta,
           ...
           colsample_bytree,
           gamma,
           compute_test_error = T) {
    
    # dtrain and dtest and defined prior to calling the function
    
    # mono_index_positve and mono_index_negative are from global
    # create mono_constraints
    mono_constraints <- rep(0, ncol(dtrain) - 1)
    mono_constraints[mono_index_positve] <- 1
    mono_constraints[mono_index_negative] <- -1
    
    # convert tibble to DMatrix
...
final_xgb_model <- function(optObj, 
                            region, 
                            season, 
                            outer_i) {
  ...
  # get optimal XGBoost paras, add monotone_constraints info
  Pars <- getBestPars(optObj)
  Pars[["monotone_constraints"]] <- mono_constraints
...
for (i in 1:nrow(input_df)) {
  ...
  file_names <- list(
    paste0("./data/5_Reg", region, "_Tr_", season, ".csv"),
    paste0("./data/5_Reg", region, "_Te_", season, ".csv")
  )
  ...
  for (outer_i in 1:(outer_repeats * outer_v)) {
    ...
    # experiments
    mono_index_positve <- c(1:7)
    mono_index_negative <- NULL
